...
app.get('/', (req, res) => res.json({ status: 'SlideAI online', engine: 'Gemini 2.0' }));
app.post('/generate', async (req, res) => {
  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        contents: [{ parts: [{ text: 'Você é especialista em apresentações. Retorne APENAS JSON válido sem markdown.\n\n' + prompt }] }],
